...
def main():
    ...
    out=ROOT/'december-artifacts';out.mkdir(exist_ok=True)
    ...
    flags={'WHISPER_MODEL_NAME':'medium','WHISPER_DEVICE':'cuda','ASR_ENABLED':'1','EDITDNA_USE_LLM':'1','EDITDNA_LLM_MODEL':'gpt-5.1','VISION_ENABLED':'1','BAD_TAKES_ENABLED':'1','TAKE_JUDGE_ENABLED':'1','TAKE_JUDGE_MODEL':'gpt-4o-mini','BOUNDARY_REFINER_ENABLED':'0'}
    env.update(flags,S3_PREFIX='cutsell/legacy-comparison/'+os.environ['GITHUB_RUN_ID'],LEGACY_SOURCE_KEY='cutsell/benchmark-inputs/upload-36065470982/5ae3cffb9034cc9aebbe4f645c4f1f34538f9113f05990291215c86b7163b681.mp4',UPLOAD_EXPECTED_SHA='5ae3cffb9034cc9aebbe4f645c4f1f34538f9113f05990291215c86b7163b681')
    for k,v in env.items():
        if k in names:print('::add-mask::'+v)
    private=Path(os.environ['RUNNER_TEMP'])/'december-env.json';private.write_text(json.dumps(env));private.chmod(0o600)
    (out/'configuration.json').write_text(json.dumps({'reconstructed':True,'flags':flags,'historical_commit':'9ffbdc1','boundary_refiner':'historical default off','source_sha256':env['UPLOAD_EXPECTED_SHA'],'modern_reference_run':36076863910},indent=2))
    child=dict(os.environ,CUTSELL_ENV_JSON_PATH=str(private))
    try:
        with (out/'modal.log').open('w') as log:
            call=subprocess.run(['modal','run','benchmarks/modal_editdna_december.py'],env=child,stdout=log,stderr=subprocess.STDOUT,cwd=ROOT)
        ...
        if s.get('ok'):
            ...
            client.download_file(env['S3_BUCKET'],env['S3_PREFIX']+'/edited.mp4',str(out/'EditDNA_Diciembre_RECONSTRUIDO.mp4'))
            video=out/'EditDNA_Diciembre_RECONSTRUIDO.mp4'
