...
def _semantic_best_take(
    ...
    *,
    ...
    if delete_recommended_ids == set(member_ids):
        ...
        if not usable:
            # A cross-window authority abstention means the semantic failure
            # labels themselves are disputed.  Local performance evidence is
            # corroboration, not an independent delete authority, so it may
            # not turn those disputed labels into a story-breaking removal.
            # Fail open to the already-ranked complete delivery; a genuinely
            # authoritative all-failed family still yields no realization.
            if semantic_comparative_authority in {
                AUTHORITY_ABSTAIN_CONFLICT,
                AUTHORITY_ABSTAIN_INCOMPLETE_CONTEXT,
            }:
                complete_ids = [cid for cid in member_ids if by_id[cid].complete_idea is not False]
                if local_selected_clip_id in complete_ids:
                    if terminal_confidence_out is not None:
                        terminal_confidence_out["terminal_besttake_confidence"] = _terminal_confidence(
                            _TERMINAL_CONFIDENCE_UNKNOWN,
                            "authority_abstain_all_failed_fail_open",
                            _TERMINAL_CONFIDENCE_PROVENANCE_NO_SCORE,
                            candidate_ids=member_ids,
                            ranked_candidate_ids=(local_selected_clip_id,),
                            top_id=local_selected_clip_id,
                        )
                    return local_selected_clip_id, None, "authority_abstain_all_failed_fail_open"
            if terminal_confidence_out is not None:
