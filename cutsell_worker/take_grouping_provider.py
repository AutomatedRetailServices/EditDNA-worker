...
def reconcile_semantic_idea_equivalence(
    ...
    *,
    ...
    directed_coverage_pairs = frozenset(
        ...
        for failed_id, delivery_id in failed_retry_coverage_pairs
        if failed_id in take_map
        and delivery_id in take_map
        and failed_id in group_index_of
        and delivery_id in group_index_of
        and group_index_of[failed_id] != group_index_of[delivery_id]
        and take_map[failed_id].source_asset_id == take_map[delivery_id].source_asset_id
        and take_map[delivery_id].start >= take_map[failed_id].end
    )
    coverage_candidate_pairs = tuple(
        (
            group_index_of[failed_id], group_index_of[delivery_id],
            failed_id, delivery_id,
        )
        for failed_id, delivery_id in sorted(directed_coverage_pairs)
    )
    coverage_pair_keys = {
        frozenset((failed_id, delivery_id))
        for failed_id, delivery_id in directed_coverage_pairs
    }
    candidate_pairs = (*coverage_candidate_pairs, *(
        pair for pair in candidate_pairs
        if frozenset((pair[2], pair[3])) not in coverage_pair_keys
    ))
    if protected_ids:
        ...
        directed_coverage_pairs = frozenset(
            pair for pair in directed_coverage_pairs
            if pair[0] not in protected_ids and pair[1] not in protected_ids
        )
    ...
    for pair in candidate_pairs:
        ...
        # A pair admitted only through the distant failed-attempt coverage
        # bridge must never inherit the ordinary deterministic-restart fast
        # path.  Its additional reach is authorized solely by the explicit
        # semantic no-conflict + directional-coverage verdict below.
        if (left_id, right_id) in directed_coverage_pairs:
    ...
    if arbiter is None:
        ...
        priority_pairs = tuple(
            ...
            if (pair[2], pair[3]) in directed_coverage_pairs
        )
        ordinary_pairs = tuple(pair for pair in remaining_pairs if pair not in priority_pairs)
        ordinary_ranked = (
            _rank_candidate_pairs_with_marks(ordinary_pairs, take_map, per_group_cap=0)
            if ordinary_pairs else ()
        )
        usage: Counter = Counter()
        priority_first: list[tuple[tuple[int, int, str, str], bool]] = []
        priority_deferred: list[tuple[tuple[int, int, str, str], bool]] = []
        for pair in priority_pairs:
            left_index, right_index = int(pair[0]), int(pair[1])
            if (
                usage[left_index] >= _PAIR_BUDGET_PER_GROUP_CAP
                or usage[right_index] >= _PAIR_BUDGET_PER_GROUP_CAP
            ):
                priority_deferred.append((pair, True))
                continue
            priority_first.append((pair, False))
            usage[left_index] += 1
            usage[right_index] += 1
        ordinary_first: list[tuple[tuple[int, int, str, str], bool]] = []
        ordinary_deferred: list[tuple[tuple[int, int, str, str], bool]] = []
        for pair, _ in ordinary_ranked:
            left_index, right_index = int(pair[0]), int(pair[1])
            if (
                usage[left_index] >= _PAIR_BUDGET_PER_GROUP_CAP
                or usage[right_index] >= _PAIR_BUDGET_PER_GROUP_CAP
            ):
                ordinary_deferred.append((pair, True))
                continue
            ordinary_first.append((pair, False))
            usage[left_index] += 1
            usage[right_index] += 1
        marked = (
            tuple(priority_first)
            + tuple(ordinary_first)
            + tuple(priority_deferred)
            + tuple(ordinary_deferred)
        )
    ...
    for pair_index, (left_group_index, right_group_index, left_id, right_id) in enumerate(truncated):
        ...
        coverage_bridge = (left_id, right_id) in directed_coverage_pairs
        coverage_safety = pair_safety_by_pair_index(result).get(pair_index) if result is not None else None
        ...
        if (coverage_bridge and not coverage_bridge_confirmed) or (
            not coverage_bridge and not same_idea
        ):
        ...
        audit.append({
            ...
            **({"accepted_by": "failed_attempt_directional_coverage"}
               if coverage_bridge_confirmed else {}),
        })
    ...
    if merged_count == 0:
        return groups, {
            ...
            "failed_retry_coverage_pair_count": len(directed_coverage_pairs),
