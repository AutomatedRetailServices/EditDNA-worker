...
evidence. Hybrid cleanup must not immediately delete that same paragraph from a medium
confidence semantic + local-motion signal. This wrapper reapplies the exact conservative
story-coverage contract after Hybrid/semantic-fragment cleanup.

Only medium-confidence Hybrid deletions are eligible for rescue. Very-high-confidence
failed/BTS classifications remain authoritative, and short/repetitive debris is never
rescued because it does not satisfy the story-coverage guard.
"""
...
def restore_hybrid_story_coverage(
    ...
    *,
    ...
    """Restore unique story paragraphs deleted only on non-hard Hybrid evidence."""
