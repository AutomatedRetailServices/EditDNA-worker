...
def _env_float(k, d):
    ...
    try: return float(v[0]) if v else d
    except: return d
...
def _ffprobe_duration(path: str) -> float:
    code, out, err = _run([FFPROBE_BIN,"-v","error","-show_entries","format=duration","-of","default=nokey=1:noprint_wrappers=1",path])
    try: return float(out.strip()) if code==0 else 0.0
    except: return 0.0

def _download_to_tmp(url: str) -> str:
    local_path = _tmpfile(suffix=".mp4")
    code, out, err = _run(["curl","-L","-o",local_path,url])
    if code != 0: raise RuntimeError(f"curl failed {code}: {err}")
    return local_path

FILLERS = {"uh","um","like","so","okay","sorry"}
RETRY_MARKERS = ("wait","hold on","let me start again","start over","no no","redo","take two","i mean","actually")

def _is_retry_or_filler(txt: str) -> bool:
    low = txt.lower()
    if any(m in low for m in RETRY_MARKERS): return True
...
def _clamp_span(s: float, e: float) -> Tuple[float,float]:
    dur = e - s
    if dur <= 0: return s, s
    if dur > MAX_TAKE_SEC: return s, s + MAX_TAKE_SEC
    return s, e
...
def _segments_to_takes(segments: List[Dict[str,Any]]) -> List[Take]:
    ...
    for i, seg in enumerate(segments, start=1):
        ...
        while (e - s) > MAX_TAKE_SEC:
            takes.append(Take(id=f"T{i:04d}_{len(takes)+1:02d}", start=s, end=s+MAX_TAKE_SEC, text=txt))
            s += MAX_TAKE_SEC
        s, e = _clamp_span(s, e)
        if (e - s) >= MIN_TAKE_SEC:
...
def _drop_retries_and_trash(takes: List[Take]) -> List[Take]:
    out = []; seen = set()
    for t in takes:
        if _is_retry_or_filler(t.text): continue
        norm = "".join(c.lower() for c in t.text if (c.isalnum() or c.isspace())).strip()
        if norm in seen: continue
        seen.add(norm); out.append(t)
    return out

def _merge_adjacent_by_flow(takes: List[Take], max_chain=3) -> List[Take]:
    if not takes: return []
...
def _pick_story_in_order(merged: List[Take], max_len: float) -> List[Take]:
    story, total = [], 0.0
    for t in merged:
        if total + t.dur > max_len: break
        story.append(t); total += t.dur
...
def _export_video(src: str, story: List[Take]) -> str:
    ...
    for idx, t in enumerate(story, start=1):
        part = _tmpfile(suffix=f".part{idx:02d}.mp4"); parts.append(part)
        _run([FFMPEG_BIN,"-y","-ss",f"{t.start:.3f}","-i",src,"-t",f"{t.dur:.3f}",
              "-c:v","libx264","-preset","fast","-crf","23","-pix_fmt","yuv420p","-g","48",
              "-c:a","aac","-b:a","128k",part])
    with open(listfile,"w") as f:
        for p in parts: f.write(f"file '{p}'\n")
    final = _tmpfile(suffix=".mp4")
    _run([FFMPEG_BIN,"-y","-f","concat","-safe","0","-i",listfile,
          "-c:v","libx264","-preset","fast","-crf","23","-pix_fmt","yuv420p","-g","48",
          "-c:a","aac","-b:a","128k",final])
    if BURN_CAPTIONS:
        srt = _write_srt(story)
        burned = _tmpfile(suffix=".mp4")
        _run([FFMPEG_BIN,"-y","-i",final,"-vf",f"subtitles={srt}",
              "-c:v","libx264","-preset","fast","-crf","23","-pix_fmt","yuv420p","-g","48",
              "-c:a","aac","-b:a","128k",burned])
        return burned
    return final
...
def run_pipeline(*, session_id: str, file_urls: List[str], portrait: bool,
                 funnel_counts: Union[str, Dict[str,int], None],
                 max_duration: float, s3_prefix: Optional[str] = None, **kwargs) -> Dict[str,Any]:
    if not file_urls:
        return {"ok": False, "error": "no input files"}
    if isinstance(funnel_counts, str):
        try: funnel_counts = json.loads(funnel_counts)
        except Exception: funnel_counts = None
    ...
    story = _pick_story_in_order(merged, float(max_duration or MAX_DURATION_SEC))
    ...
    clips = [{
        ...
        "score": 2.5, "face_q": t.face_q, "scene_q": t.scene_q,
        "vtx_sim": t.vtx_sim, "chain_ids": t.chain_ids or [], "text": t.text
    } for t in story]
    slots = {
        "HOOK": [{
            "id": t.id, "start": t.start, "end": t.end, "text": t.text,
            "meta": {"slot":"HOOK","score":2.5,"chain_ids":t.chain_ids or []},
            "face_q": t.face_q, "scene_q": t.scene_q, "vtx_sim": t.vtx_sim,
            "has_product": False, "ocr_hit": 0
        } for t in story],
        "PROBLEM": [], "FEATURE": [], "PROOF": [], "CTA": []
    }
    return {
        "ok": True,
        ...
        "clips": clips, "slots": slots,
        "semantic": True, "vision": False, "asr": True,
