...
def _env_int(k, d): 
    v = (os.getenv(k) or "").split("#")[0].strip().split()[:1]
    try: return int(v[0]) if v else d
    except: return d
...
def _segments_to_takes(segments: List[Dict[str,Any]]) -> List[Take]:
    ...
    for i, seg in enumerate(segments, start=1):
        ...
        while (e - s) > MAX_TAKE_SEC:
            ...
            takes.append(Take(id=f"T{i:04d}_{len(takes)+1:02d}", start=chunk_s, end=chunk_e, text=txt))
            s = chunk_e
        s, e = _clamp_span(s, e)
        if (e - s) >= MIN_TAKE_SEC:
...
# ---------------- PUBLIC ENTRY ----------------
def run_pipeline(*, session_id: str, file_urls: List[str], portrait: bool, funnel_counts: str, max_duration: float, **kwargs) -> Dict[str,Any]:
    if not file_urls: return {"ok": False, "error": "no input files"}

    raw_local = _download_to_tmp(file_urls[0])

    # 1) REAL ASR
    segs = transcribe_segments(raw_local)  # ← replaces TEMP PLACEHOLDER
    ...
    clips_block = [{
        "id": t.id, "slot": "STORY",
        "start": t.start, "end": t.end, "score": 2.5,
        "face_q": t.face_q, "scene_q": t.scene_q, "vtx_sim": t.vtx_sim,
        "chain_ids": t.chain_ids or [], "text": t.text
    } for t in story]

    slots_block = {  # keep HOOK list so frontend doesn’t break; we’ll add real slotting next step
        "HOOK": [{
    ...
    return {
        ...
        "clips": clips_block, "slots": slots_block,
