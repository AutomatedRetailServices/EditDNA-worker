...
def sanitize_clean_cut_discard_diagnostics(diagnostics: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    sanitized: List[Dict[str, Any]] = []
    for item in diagnostics or []:
        clean = dict(item)
        source_index = int(clean.get("source_index", 0))
        clean["source_index"] = source_index
        clean["source_local"] = sanitize_source_identifier(clean.get("source_local"), source_index)
