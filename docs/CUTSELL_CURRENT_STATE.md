...
### Native AV QA audit — distinguish rejection from editorial quality

See CUTSELL_NATIVE_AV_QA_AUDIT.md. Exact tested-module replay proves the old AV region error also covers out-of-range confidence, reversed/negative intervals and hallucinated ends, not only encoder padding. Exact live offending values were not retained. The padding fix is reproducible but not confirmed as both runs' cause. Native AV remains advisory classifier context; positive AV + .99 mixed label without independent local corroboration leaves the take unchanged in a controlled fixture. A maximal 12-region context fixture retains only one under the 1,800-character cap (not a live measured omission). Current edge trims do not handle interior mixed speech. Earlier baseline prosody/guard metrics showed narrow/no eligibility and zero winner changes; aborted AV runs did not reach those stages. No engine change, paid calls or renders in this QA pass. Integration remains unqualified.


## Native AV replay, candidate evidence and bounded interior preparation — 2026-09-25

Authorized scope: offline integration/QA on feat/gpt-whisperx-video00; no additional paid qualification or production activation.

Observed failure: both authorized native AV runs aborted before selection and rendering. Their rejected response values were not retained. Earlier non-native baselines removed one take with whole_video_bad_take:wrong_take in each source, while remaining MOV fragments were preserved as valid_or_uncertain_speech. A MOV pep-talk winner was already DECISIVE_BY_ELIMINATION, so the bounded prosodic comparison was ineligible. This is a classification/grouping/eligibility gap, not evidence that the engine automatically learns Video00. A scoped AST string-constant search found no Video00/source-hash/product/quoted-problem-phrase exceptions; this is not an exhaustive proof against overfitting.

Implementation:
- Added a pure versioned AV response contract, constrained response JSON schema and offline replay. Visible returned text, source hash/durations, model, usage and reserved budget are retained before validation; rejected confidence and timestamps have distinct errors. No model thinking, media bytes or credentials are captured. Required AV failures carry audit records into benchmark error artifacts; successful diagnostics retain them too.
- Linked advisory AV observations to source/time/real word spans for each candidate. Up to three overlaps are carried with an explicit omitted count. Compact global region references precede verbose text when fitting existing payload limits; a 12-region verbose fixture retains all references under the 1,800-character cap. This is fixture coverage, not proof of live model recall.
- Added optional disjoint inclusive recording_word_ranges for mixed candidates. Interior removal requires complete sentence boundaries, matching source identity, real aligned words, >=.97 recording certainty, agreement across returned windows, independent local corroboration of each removed fragment, and meaningful contiguous retained pieces. Retained pieces reenter ordinary grouping/selection independently. No cross-take speech stitching or AV self-corroboration. Prefix/suffix behavior remains supported.
- Every mixed-trim candidate now records whether it was kept and why, including missing boundaries, insufficient confidence, disagreement, alignment mismatch and missing independent local evidence. Arbitrary intraphrase self-corrections remain protected; this does not solve every mixed take or retry grouping failure.

Verification: 101 focused tests passed. Broader relevant regression: 940 passed, five failed. All five failures reproduce unchanged at parent e70543b: four legacy workflow-parity assertions in test_video00_modal_hybrid_semantic_parity.py and one incomplete-retry coverage expectation in test_hybrid_story_guard_incomplete_retry.py. They remain open; no all-green or release-ready claim. New EN/ES fixtures cover inter-sentence preparation, source-word partition, unsafe intraphrase preservation, independent corroboration and end-to-end retention of both audience facts after an interior removal. AV replay and dense payload fixtures use stub responses, not the missing live responses.

Limits: prosodic/BestTake eligibility has been diagnosed, not broadened. No live Gemini call, GPU benchmark or new render in this change. Perceptual accuracy, model support for the new constrained schema in a live request, final cut quality and generalization across unseen videos remain unqualified. Canonical deletion authority, product scope and production topology are unchanged.


## Retry relationship and directional coverage before removal — 2026-09-25

User requested checking whether a clean retake replaces an abandoned attempt BEFORE closing the keep/delete decision, and verifying the hypothesis against Video00 and the last MOV.

Observed code defect: hybrid_retry_winner_authority already checks a failed label, retry_setup event, nearby same-source winner and lexical retry relationship, but its numeric gate tolerated missing numbers and it did not require a complete replacement or directional claim coverage. It chose one best candidate and stopped after a prior identity rejection, leaving other valid candidates unexamined. This can both accept an unsafe replacement and miss a usable alternative. The window-local replacement scan and early deterministic deletions are separate routes and are not redesigned by this change.

Implementation:
- Added retry_replacement_coverage, reusing existing semantic_claims and contradiction_signal contracts. Require source/chronology compatibility, complete replacement, no known mixed/recording-only role or contradictory failed/BTS window label, preservation of original numbers, no detected contradiction, and coverage of each extracted original sentence claim. Respect the existing directional identity rejection. This is a conservative deterministic check, not proof of unrestricted semantic equivalence or perceptual cleanliness.
- Integrated this check immediately before removal in enforce_proven_retry_winners. Failed/nearby/retry_setup/winner gates remain; a failed first candidate no longer hides subsequent eligible retakes. Every declined comparison records the missing coverage or cleanliness reason; a successful deletion names the verified peer.
- Added a cross-window review of the complete classified pool before downstream composite resolution. It scans the existing nearby retry relationship without extra provider calls. This review is explanatory evidence, not a new deletion authority, and does not force groups to merge.
- Added scripts/audit_retry_coverage.py for exact-identity offline reconstruction of saved benchmark evidence. It refuses missing completeness metadata or unrecoverable text hashes; no inference from the final edit and no counterfactual render claim.

Saved-run audit (docs/evidence/retry_coverage_20260925/):
- MOV: recovered 14 of 18 classified candidates. Of two nearby supported comparisons, one was rejected because the proposed replacement was marked mixed; one passed the existing claim-coverage contract. The latter still contains apparent preparation in its transcript but was labeled audience in the saved evidence. This exposes an upstream classification limitation: coverage and absence of a recorded mixed label cannot certify genuinely clean delivery.
- Video00: recovered 36 of 42 candidates; one supported sonography retry pair passed coverage. Other candidates were not all recoverable and not all failed labels had an eligible nearby peer. This is partial evidence supporting the user's hypothesis, not a quantified explanation of every bad edit.
