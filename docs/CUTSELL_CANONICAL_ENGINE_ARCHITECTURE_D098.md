...
| Retry-family / `IdeaClusterer` grouping (`take_grouping.py`, `take_grouping_provider.py`, D-097.A restart-evidence rules incl. D-097.12's `incomplete_attempt_completed_by_retry`) | L4 Attempt/Temporal Relationship Understanding | EXISTING | D-097.12 offline-proven stomach-family fix; D-097.A family completeness RAW-proven | Multimodal (non-transcript) relationship evidence; explicit `retry_of`/`corrects`/`complements` typed relationships beyond restart-evidence kinds | CONSOLIDATE LATER |
