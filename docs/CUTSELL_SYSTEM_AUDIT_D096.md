...
| Duplication entries 16 + 19 (`hybrid_complementary_delivery_guard`, `hybrid_composite_best_take`) predate D-097 §3: a `failed` 0.95 abandoned start restored on inflected "unique" tokens and composed with another abandoned attempt to replace a `winner` 0.95 clean retry before IdeaClusterer ran (RAW 34042123557) | IMPLEMENTED (D-097.6 R7) | `_semantically_unusable` (failed >= `_SEMANTIC_FAILED_THRESHOLD`) refused for unique-tail restore, performance-only rescue and composite membership; the winner stands |
| D-062.2 convenience-valve block, new instance (RAW 34043247473): StoryValidator blocked Freeze over a take clean-cut removed on a multimodal `wrong_take` confirmation (0.97); the pre-group credit trusted only the arbiter, which answered the wrong question | IMPLEMENTED (D-097.7 R8) | `_pre_group_wrong_take_credit`: wrong_take >= 0.85 + clean-cut removal for that reason + D-097.1 retry adjacency to a selected delivery suppress the coarse vocabulary signal only; atoms untouched |
| IdeaClusterer pair budget (`max_pairs_per_request` 14 of 55) spent on adjacent narrative pairs: proximity weighed 1.0 and the overlap counted stopwords (RAW 34043967265: the abandoned stomach pair never asked; an incomplete attempt played) | IMPLEMENTED (D-097.8 R9) | `_raw_content_overlap` on D-048 content tokens; `_PROXIMITY_RANK_WEIGHT = 0.25`; budget unchanged |
| D-081 deferred semantic delete never applied to a SINGLETON (RAW 34043967265: the video ended on a corroborated `bts` 0.95 "No, no, no, no, no.") | IMPLEMENTED (D-097.8 R10) | `_semantic_best_take`: lone `bts` >= 0.85 + deterministic unusable -> `single_bts_unusable` (no-usable-realization, recorded); lone `failed` unchanged (D-097.B) |
| D-097.8 R10's drop reused the D-097.B `no_usable_realization` row without a basis; `universal_clean_cut` read every such row as a dropped IDEA -> `story_completeness = incomplete`, clean MP4 refused (RAW 34045158712: "¡Vamos!", both references remove it) | IMPLEMENTED (D-097.9 R11) | judge row `no_usable_realization_basis`; `universal_clean_cut.derive_story_completeness` counts dropped idea families only; `stage_status` lists idea vs bts-singleton ids; StoryValidator rows/findings carry `basis` |
| S-9 / P-46 collision: `editorial_slot_resolution_install._install_semantic_pair_budget_coverage` wrapped `_rank_candidate_pairs` at import with a coverage-first re-order (one pair per group first) -- a second authority over the arbiter budget order that defeated D-097.8 R9 (RAW 34045158712: 0.02-0.03 zero-evidence neighbours asked; a 0.945 same-opening pair demoted to 12th in replay) | IMPLEMENTED (D-097.9 R12) | wrapper retired from `install_editorial_slot_resolution` (function kept, deactivated); its fairness concern (D-042 dense-cluster test) lives inside the one authority as `_PAIR_BUDGET_PER_GROUP_CAP = 2` (first pass capped per group, deferred pairs behind, both in score order); `reconcile_semantic_idea_equivalence` records `ranked_pair_budget` (scores, cap marks, order spent) + `pair_order_authority` |
| Stomach aside kept on a run-to-run change of the multimodal confirmation KIND (`wrong_take` 0.97 in 34043967265 -> `retry_setup` 0.86 in 34045158712); the attempt -> own 0.26 s restart -> same-opening clean delivery chain is consumed by no authority for a `winner`-labelled singleton; deterministic restart evidence needs 4 opening tokens (3 shared), the false start exceeds the short-prefix cap, and the pair ranks 23rd of 101 | ESCALATION A (D-097.10 R13) | RAW 34047064840: all three pairs asked (R12); arbiter same 0.90 / same 0.85 / NOT-same 0.85 ("adds the diagnosis detail"); D-085 probe declined at 0.85 -> clean delivery split off; F3b inapplicable (0.85 < 0.90). Levers are semantic-authority policy (arbiter prompt/D-042 policy, D-085 floor): Product Owner |
| Post-render QA layers read PRE-render boundaries: the perceptual reviewer rebuilt segments from `input_boundary_state` (pre-tighten ends) and flagged reset debris the renderer had already cut (9 of 13 exit findings on RAW 34047064840); the ladder's physical view depended on MP4 correlation alone | IMPLEMENTED (D-097.10 R14) | `universal_clean_cut_validation.segments_as_rendered` (recorded `renderer_trailing_trims` applied before review; `renderer_trims_applied` recorded); `video00_quality_ladder.recorded_renderer_ends` + `physical_engine_result` recorded-trims-first with `physical_source`; workflows print the physical Level-1 headline |
| `take_segmentation._ends_sentence` read a trailing ellipsis as a full stop, so a visibly cut-off attempt ("... me diagnosticaron con...") carried `complete_idea=True` into ranking, clean-cut, Resolver usability/claim rules and the workaround guard `cross_group_truncated_winner_authority` (RAW 34048444463: 2 of 39 takes) | IMPLEMENTED (D-097.11 R15) | `_trails_off`: a trailing "..."/"…" is never a sentence end and is an open tail; join rules unchanged (contiguous-only) |
| Stomach family decided by the arbiter's run-to-run verdict on the same pair (SAME 0.95 -> NOT-same 0.85 -> NOT-same 0.90 across RAWs 34045158712 / 34047064840 / 34048444463; "incomplete vs complete" reasons the prompt already excludes); every deterministic tier exhausted | ESCALATION A (D-097.11 R13, evidence #3) | options: (i) deterministic incomplete-attempt-restarted rule, (ii) prompt clause + isolation probes (C), (iii) accept fail-closed keep for human W+L |
