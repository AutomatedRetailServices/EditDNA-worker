...
_BASE_PATH = Path(__file__).with_name("round14_capacity_failover.py")
_spec = importlib.util.spec_from_file_location("round14_capacity_failover", _BASE_PATH)
...
def upload_source_bundle(base_env: dict) -> tuple[str, str, str]:
    ...
    key = f"cutsell/bootstrap/round17/{base.RUN_ID}/{base.SOURCE_SHA}/source-bundle.tgz"
    s3 = boto3.client(
        "s3",
        region_name=region,
...
urllib.request.urlretrieve(os.environ['CUTSELL_SOURCE_BUNDLE_URL'], '/tmp/cutsell-source.tgz')
...
def find_public_worker(base_env: dict, mount: str, bundle_url: str):
    ...
    env.update({"CUTSELL_SOURCE_BUNDLE_URL": bundle_url, "CUTSELL_EXPECTED_SOURCE_SHA": base.SOURCE_SHA, "PYTHONPATH": "/app"})
    gpus = [
        "NVIDIA GeForce RTX 5090", "NVIDIA L40S", "NVIDIA RTX 6000 Ada Generation",
        "NVIDIA A100 80GB PCIe", "NVIDIA A100-SXM4-80GB", "NVIDIA L40",
        "NVIDIA GeForce RTX 4090", "NVIDIA A40", "NVIDIA RTX A6000", "NVIDIA L4",
    ...
    for cloud in ("SECURE", "COMMUNITY"):
        ...
        if r.status_code != 201:
            ...
            continue
        info = r.json(); pid = str(info.get("id") or ""); created = time.time()
        ...
        runtime_seen = False; runtime_deadline = time.time() + 900; last = None
        while time.time() < runtime_deadline:
            state = base.api("GET", f"/pods/{pid}")
            if state.status_code == 200:
                data = state.json(); runtime = data.get("runtime"); runtime_seen = isinstance(runtime, dict)
                snap = {"desired": data.get("desiredStatus"), "runtime": runtime_seen, "last": data.get("lastStatusChange"), "gpu": ((data.get("gpu") or {}).get("displayName") if isinstance(data.get("gpu"), dict) else None), "machine_id": ((data.get("machine") or {}).get("id") if isinstance(data.get("machine"), dict) else None)}
                if snap != last:
                    print("public_pod_state=" + json.dumps({"pod": pid, "cloud": cloud, **snap}, ensure_ascii=False), flush=True); last = snap
                if runtime_seen: break
            time.sleep(15)
        if not runtime_seen:
            attempts.append({"cloud": cloud, "pod": pid, "result": "no_runtime_public_image"}); base.delete_pod(pid); continue
        worker_deadline = time.time() + 1500; chosen = None
        while time.time() < worker_deadline:
            now = datetime.now(timezone.utc)
            for worker in Worker.all(connection=base.redis):
                hb = worker.last_heartbeat
                if not hb or worker.name in before: continue
        ...
        attempts.append({"cloud": cloud, "pod": pid, "result": "runtime_without_rq_after_bootstrap"}); base.delete_pod(pid)
...
def main() -> None:
    ...
    try:
        ...
        chosen_pid = pid; Path("pod-id.txt").write_text(pid)
        status.update({"status": "worker_live", "pod_id": pid, "cloud": cloud, "gpu": gpu, "worker": worker})
        json.dump(status, open("round17-status.json", "w"), indent=2)
        ...
        json.dump(status, open("round17-status.json", "w"), indent=2)
        print("ROUND17_GOLD_PASS=true", flush=True)
    finally:
        json.dump(attempts, open("capacity-attempts.json", "w"), ensure_ascii=False, indent=2)
        if chosen_pid: base.delete_pod(chosen_pid)
        try:
            s3 = boto3.client("s3", region_name=base_env.get("AWS_REGION") or "us-east-1", aws_access_key_id=base_env["AWS_ACCESS_KEY_ID"], aws_secret_access_key=base_env["AWS_SECRET_ACCESS_KEY"])
            s3.delete_object(Bucket=bucket, Key=key); print("source_bundle_deleted=true", flush=True)
