...
class TestDesignOnlyNoImplementation:
    ...
    def test_17_no_new_canonical_id_minted_by_this_gate(self):
        ...
        # id-minting function to live in. D-235P/D-235S/D-235T (SEPARATELY-
        # authorized, later gates) went on to add
        # `shared_attempt_word_identity.py`, `lost_atom_reviewer_
        # finding_provenance.py`, and `lost_atom_repair_suppression.py` --
        # none of the two specific filenames D-235O's own design text
        # proposed (test_16, still both absent) -- so this snapshot is
        # widened to include those later, authorized additions rather than
        # pretending the family can never grow; it still proves D-235O's
        # OWN gate, by itself, minted zero new canonical ids in zero new
        # files.
        import os
        new_files = [
