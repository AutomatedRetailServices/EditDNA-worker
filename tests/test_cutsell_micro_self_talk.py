...
def test_short_expletive_between_attempts_is_removed_with_visual_break():
    ...
    kept, removed, diagnostics = apply_micro_self_talk_cleanup(
        (before, reaction, after), break_context()
    )
...
def test_isolated_profanity_reaction_survives():
    reaction = take("reaction", "oh shit", 0.0, 1.0)
    kept, removed, diagnostics = apply_micro_self_talk_cleanup((reaction,), break_context())
    assert kept == (reaction,)
    assert removed == ()
    assert diagnostics == ()


def test_reaction_between_attempts_survives_without_visual_break():
