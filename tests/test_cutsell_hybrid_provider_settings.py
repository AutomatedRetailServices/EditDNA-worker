...
def test_key_or_provider_name_alone_cannot_enable_paid_calls():
    settings = load_hybrid_provider_settings({
        "GOOGLE_API_KEY": "present-but-not-sufficient",
...
def test_escalation_is_dynamic_not_every_call():
    settings = HybridProviderSettings()
    assert settings.should_escalate(local_confidence=0.50, conflict_score=0.10)
    assert settings.should_escalate(local_confidence=0.80, conflict_score=0.80)
    assert not settings.should_escalate(local_confidence=0.80, conflict_score=0.20)
