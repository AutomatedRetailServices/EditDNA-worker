...
def _assert_out_trim(earlier_text):
    earlier = take("earlier", earlier_text, 0.0, 3.0)
...
def test_boundary_word_is_kept_and_repeated_retry_suffix_is_trimmed():
    _assert_out_trim("were one of those people that use your desk to pop your back")


def test_contraction_variant_in_earlier_retry_still_matches_asr_were():
    _assert_out_trim("we're one of those people that use your desk to pop your back")
...
def test_same_short_boundary_without_word_timing_fails_open():
    ...
    trimmed, diagnostics = trim_trailing_retry_restarts((earlier, previous, tail), (earlier, previous, tail))
...
def test_valid_contiguous_transition_without_earlier_retry_is_unchanged():
    ...
    previous = take("previous", "drop the link below if you want to check them", 37.72, 40.48)
    tail = take(
        "tail", "out and let me show you the zipper", 40.48, 41.70,
...
def test_noncontiguous_short_retry_fragment_is_not_boundary_trimmed():
    ...
    tail = take(
        "tail", "out if you were one of those", 40.48, 41.54,
        ("out", "if", "you", "were", "one", "of", "those"),
    )
    trimmed, diagnostics = trim_trailing_retry_restarts((earlier, previous, tail), (earlier, previous, tail))
    assert trimmed[-1] == tail
