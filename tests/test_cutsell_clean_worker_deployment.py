...
def test_feature_runpod_paid_deploy_cannot_trigger_from_a_commit_and_needs_run30_token():
    deploy = Path(".github/workflows/cutsell-feature-runpod-one-shot-deploy.yml").read_text()
    assert "workflow_dispatch:" in deploy
    assert "approval_token:" in deploy
    assert 'test "$APPROVAL_TOKEN" = "RUN30"' in deploy
...
def test_hybrid_unseen_benchmark_is_manual_and_requires_same_run30_token():
    ...
    assert 'test "$APPROVAL_TOKEN" = "RUN30"' in benchmark
