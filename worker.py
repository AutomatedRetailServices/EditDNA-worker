# worker.py — concatenate clips with ffmpeg, upload MP4 to S3 (uses s3_utils.upload_file)
...
from s3_utils import upload_file  # uses your existing helper that returns s3://...
...
def job_render(*args, **kwargs) -> dict:
    """
    Accepts either:
      - job_render({ 'session_id': str, 'files': [urls], 'output_prefix': str, 'portrait': bool })
      - job_render(session_id, files, output_prefix, portrait=True)  (legacy positional)
    Produces a single concatenated MP4 uploaded to S3 and returns s3:// URL.
    """
    # Support dict payload or positional
    ...
    try:
        ...
        # 2) upload to S3 (returns s3://bucket/key)
        ...
        return {
            ...
            "mode": "concat",
            "output_s3": s3_uri,         # web adds a presigned HTTPS in /jobs/{id}
            "inputs": files,
        }
...
def job_render_chunked(*args, **kwargs) -> dict:
    # Currently same behavior; kept for compatibility
