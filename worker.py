...
def _render_concat(session_id: str, files: List[str], workdir: str) -> str:
    ...
    # 3) ffmpeg args
    # NOTE: -ignore_unknown is a flag (no value). Giving "1" breaks with: “Unable to
    # find a suitable output format for '1'”.
    args = [
        "ffmpeg",
        "-y",
...
# --------- upload (optional to S3 if output_prefix is s3://...) ---------
def _maybe_upload_to_s3(local_path: str, output_prefix: str) -> str:
    ...
    Accepts either 'editdna/outputs' (returns local path) or 's3://my-bucket/editdna/outputs'.
    """
